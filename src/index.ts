...
function cadastrarLivro() {
    ...
    const livro = new Livro(titulo, autor, editora, isbn, ano, localizacao,);
...
function editarItem(tipo: string) {
    ...
    do {
        disponibilidade = readlineSync.question('Nova Disponibilidade (Disponivel, Indisponivel, Alugado, Reservado): ') as Disponibilidade;
        if (!disponibilidadesPermitidas.includes(disponibilidade)) {
            console.log(`Disponibilidade inválida. Valores permitidos são: ${disponibilidadesPermitidas.join(', ')}`);
        }
    } while (!disponibilidadesPermitidas.includes(disponibilidade));
